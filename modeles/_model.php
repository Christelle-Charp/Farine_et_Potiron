...
class _model{
    ...
    function get($nom) {
        ...
        if (isset($this->liensMultiples[$nom])){
            ...
            // Ajoute le namespace complet si ce n'est pas déjà qualifié
            if ($typeObjet[0] !== '\\') {
                $typeObjetComplet = '\\modeles\\' . $typeObjet;
            } else {
                $typeObjetComplet = $typeObjet;
            }
            ...
            if (isset($this->valeurs[$nom]) && is_array($this->valeurs[$nom])) {
                ...
                foreach($this->valeurs[$nom] as $id) {
                    ...
                    $tabObjets[$id] = new $typeObjetComplet($id);
                }
            }
            return $tabObjets;
        ...
        if (isset($this->liens[$nom])) {
            ...
            $objetPointe = new $typeObjetComplet($this->valeurs[$nom]);
            return $objetPointe;
        }
    ...
    function loadLiensMultiples(){
       ...
       //Parcourir le tableau des liens multiples
        foreach ($this->liensMultiples as $nomChamp => $typeObjet) {
            ...
            $idChampLie = (new $typeObjetComplet())->idChamp;
